...
type FileOperationInput = z.infer<typeof inputSchema>;
type FileOperationOutput = z.infer<typeof outputSchema>;
...
export const fileOperationsTool = createTool({
    ...
    inputSchema,
    outputSchema,
    execute: async ({ context }): Promise<FileOperationOutput> => {
        const { containerId, operation, filePath, content, targetPath } = context as FileOperationInput;
        ...
        switch (operation) {
            ...
            case "write":
                ...
                return await new Promise<FileOperationOutput>((resolve, reject) => {
